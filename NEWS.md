...
* overhauled and finalized the extract rwl logic: new `extract_rwl()` + `scale_for_tucson()` (then write with `dplR::write.tucson()`), also added an `Export rwl` button to the flags Shiny app. The previous `create_rwl()` is kept as a legacy function.
...
* add vignette stubs for the Shiny apps
