...
matches list names against the known component names, drops any
unrecognised names with a warning, and calls
...
[`print()`](https://rdrr.io/r/base/print.html) shows the image-level
coverage (images, slides, woodpieces, trees, sites, species, year range)
and the status of each optional component — present with row count, or
flagged as not provided.
...
validates each component in turn and returns `TRUE` invisibly when done:
