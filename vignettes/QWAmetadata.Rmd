...
- **`$resources`**: not modified by `complete_QWAmetadata()` -- it's
  populated by TRIA at publish time, not by this package.
...
separate, standalone artifact built with `compile_resources()` and
written to its own CSV, submitted alongside a zip of the files
themselves. See `vignette("resources")` for the full workflow.
